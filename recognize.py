    ...
    for i in range(8):
        ...
        for j in range(8):
            ...
            html += '<div class="prediction" style="color: {}">{}</div>'.format(
                _confidence_color(c), format(c, ".3f")
            )
        html += "</div>"
    html += "</div>"
    html += "</div>"
...
def predict_chessboard(chessboard_img_path, options={}):
    ...
    # checking for flip
    print(
        "https://lichess.org/editor/{}".format(
            chess.Board(predicted_fen).transform(chess.flip_vertical).fen()
        )
    )
    _save_output_html(
        chessboard_img_path, predicted_fen, [p[1] for p in predictions], confidence
    )
    # print("Saved {} prediction to {}".format(chessboard_img_path, OUT_FILE))

    playing_side = input("Enter the playing side (white/black): ").strip().lower()
    if playing_side not in ["white", "black"]:
        raise ValueError("Invalid playing side. Please enter 'white' or 'black'.")
    if playing_side == "black":
        # predicted_fen = chess.Board(predicted_fen).transform(chess.flip_vertical).fen()
        predicted_fen = predicted_fen + " b"
    else:
        predicted_fen = predicted_fen + " w"
    print(predicted_fen)
    fen = predicted_fen
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-q", "--quiet", help="Only print recognized FEN position", action="store_true"
    )
    parser.add_argument(
        "-d", "--debug", help="Saves debug output to debug.html", action="store_true"
    )
    parser.add_argument("image_path", help="Path/glob to PNG chessboard image(s)")
    args = parser.parse_args()
    if not args.quiet:
        print("Tensorflow {}".format(tf.version.VERSION))
    model = load_model(NN_MODEL_PATH + ".h5")  # Load the model in HDF5 format
    ...
    if len(sys.argv) > 1:
        ...
        for chessboard_image_path in sorted(glob(args.image_path)):
            predict_chessboard(chessboard_image_path, args)
